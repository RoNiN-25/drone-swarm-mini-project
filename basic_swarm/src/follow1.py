...
while not rospy.is_shutdown():
    ...
    q = Quaternion(uav2_pose.pose.orientation.w,uav2_pose.pose.orientation.x,uav2_pose.pose.orientation.y,uav2_pose.pose.orientation.z)
    q_inv = q.inverse()
    j = q_inv.rotate([vel_x[0],vel_y[0],vel_z[0])
    # vel.linear.x = vel_x[0] + uav1_vel.linear.x
    # vel.linear.y = vel_y[0] + uav1_vel.linear.y
    # vel.linear.z = vel_z[0] + uav1_vel.linear.z
    vel.linear.x = j[0] + uav1_vel.linear.x
    vel.linear.y = j[1] + uav1_vel.linear.y
    vel.linear.z = j[2] + uav1_vel.linear.z
    pub2.publish(vel)
